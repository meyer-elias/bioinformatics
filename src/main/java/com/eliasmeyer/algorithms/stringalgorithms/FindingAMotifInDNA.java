...
/*
	...
	Given two strings s and t, t is a substring of s if t is contained as a contiguous collection of symbols in s  (as a result, t
...
public class FindingAMotifInDNA {
	...
	static void main(String[] args) throws IOException {
		new FindingAMotifInDNA().readAndProcessInput();
	}

	public void readAndProcessInput() throws IOException {
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
			DNA source = new DNA(reader.readLine().trim());
			DNA motif = new DNA(reader.readLine().trim());

			System.out.println(formatLocations(source.motifLocations(motif)));
		}
	}

	private String formatLocations(Sequence<Integer> locations) {
		StringBuilder out = new StringBuilder();
		for (Integer pos : locations) {
			if (out.length() > 0) {
				out.append(' ');
			}
			out.append(pos);
